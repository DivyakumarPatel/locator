...
class LocationBloc extends HydratedBloc<LocationEvent, LocationState> {
  ...
  void _onUpdateLocation(UpdateLocation event, Emitter<LocationState> emit) async{
    ...
    try{
      Response response = await Api().updateLocation(current_latitude: event.current_latitude, current_longitude: event.current_longitude, max_distance: event.max_distance, origin_longitude: event.origin_longitude, origin_latitude: event.origin_latitude);
